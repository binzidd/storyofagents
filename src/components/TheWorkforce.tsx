    ...
    <div className="relative w-full" style={{ maxWidth: 400, margin: "0 auto" }}>
      <svg viewBox="0 0 400 420" className="w-full h-auto" style={{ overflow: "visible" }}>
            ...
            <g key={i}>
              ...
              {edge.label && (
                <motion.text x={(x1+x2)/2+(edge.conditional?(x2<x1?-8:8):0)} y={(y1+y2)/2-4}
                  ...
                  initial={{ opacity:0 }} animate={isVisible?{opacity:1}:{opacity:0}} transition={{ delay: i*0.12+0.6 }}>
                  {edge.label}
                </motion.text>
              )}
            </g>
        ...
        {GRAPH_NODES.map((node, i) => {
          ...
          return (
            <motion.g key={node.id} initial={{ opacity:0, scale:0 }}
              animate={isVisible?{opacity:1,scale:1}:{opacity:0,scale:0}}
              transition={{ duration:0.4, delay:i*0.1 }}
              onHoverStart={() => setHoveredNode(node.id)}
              onHoverEnd={() => setHoveredNode(null)} style={{ cursor:"pointer" }}>
              {isHovered && <circle cx={cx} cy={cy} r={28} fill={node.color} opacity={0.15} />}
              {isDecision ? (
                <motion.rect x={cx-18} y={cy-18} width={36} height={36} rx={4}
                  fill={`${node.color}22`} stroke={node.color} strokeWidth={isHovered?2.5:1.5}
                  transform={`rotate(45 ${cx} ${cy})`}
                  animate={{ strokeOpacity:[0.6,1,0.6] }} transition={{ duration:2, repeat:Infinity }} />
              ) : (
                <motion.rect x={cx-36} y={cy-14} width={72} height={28} rx={isHuman?14:6}
                  fill={`${node.color}22`} stroke={node.color} strokeWidth={isHovered?2.5:1.5}
                  animate={isHuman?{strokeDashoffset:[0,-20]}:{}} strokeDasharray={isHuman?"4 2":"none"}
                  transition={{ duration:1.5, repeat:Infinity, ease:"linear" }} />
              )}
              <text x={cx} y={cy+1} textAnchor="middle" dominantBaseline="middle" fontSize={isDecision?"10":"12"}>{node.icon}</text>
              <text x={cx} y={cy+22} textAnchor="middle" fill="#1f2937" fontSize="8" fontWeight="600" fontFamily="Inter, sans-serif">{node.label}</text>
            </motion.g>
          );
        })}
      </svg>
  ...
  return (
    <div className="flex items-center gap-6 justify-center my-6">
      <div className="text-center">
        ...
        <div className="text-gray-500 text-sm mt-1 font-medium">days to close</div>
      </div>
      <div className="text-gray-300 text-4xl font-light">→</div>
      <div className="text-center opacity-40">
        <div className="text-7xl font-black text-emerald-500 leading-none">2</div>
        <div className="text-gray-500 text-sm mt-1 font-medium">target</div>
      </div>
    </div>
  );
}
              ...
              <motion.div initial={{ opacity:0, x:-40 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }}
                ...
                <motion.button onClick={playGraph}
                  ...
                  {graphPlaying ? (
                    ...
                    <><span className="text-lg">▶</span> Run the Close Cycle</>
                  )}
                </motion.button>
              ...
              <motion.div initial={{ opacity:0, x:40 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }}
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2 px-2 py-1 rounded-lg bg-emerald-500/20 border border-emerald-500/30">
                    <div className={`w-1.5 h-1.5 rounded-full ${graphPlaying?"bg-amber-400 animate-pulse":"bg-emerald-400"}`} />
                    <span className="text-emerald-400 text-xs font-mono">{graphPlaying?"running":"live graph"}</span>
                  </div>
                </div>
                <LangGraphViz isVisible={isInView || graphPlaying || graphDone} />
